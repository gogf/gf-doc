...
[server]
    ...
    ServerRoot        = "/var/www"                   # 静态文件服务的目录根路径，设置时自动开启静态文件服务。默认关闭
	...
	CookieDomain = ""     # Cookie有效域名。默认为当前设置Cookie时的域名
	# Session配置
	...
	FormParsingMemory = 1048576   # 解析表单时的缓冲区大小(Byte)，一般不需要设置。默认为1024*1024=1MB
