...
## `container`
1. `garray`
    - 简化数组使用方式，支持类似于`var garray.Array`的变量定义使用方式：https://goframe.org/container/garray/index
    - 增加`Walk`方法，用于自定义的数组元素处理方法：https://goframe.org/container/garray/index
    - 增加`ContainsI`方法，用于大小写忽略匹配的数组元素项存在性查找：https://goframe.org/container/garray/index
    - 完善单元测试，代码覆盖率`94%`；
    - 代码改进，提高性能；
    - 修复一些问题；
1. `gchan`
...
1. `glist`
    - 简化链表使用方式，支持类似于`var glist.List`的变量定义使用方式：https://goframe.org/container/glist/index
    - 完善单元测试，代码覆盖率`99%`；
1. `gmap`
    - 简化`Map`使用方式，支持类似于`var gmap.Map`的变量定义使用方式：https://goframe.org/container/gmap/index
    - 完善单元测试，代码覆盖率`81%`；
    - 代码改进，提高性能；
1. `gset`
    - 简化集合使用方式，支持类似于`var gset.Set`的变量定义使用方式：https://goframe.org/container/gset/index
    - 增加`Walk`方法，用于自定义的集合元素处理方法：https://goframe.org/container/gset/index
    - 完善单元测试，代码覆盖率`90%`；
    - 代码改进，提高性能；
1. `gtree`
...
1. `ghttp`
    ...
    - 改进`SessionId`生成的随机性，提高`Session`安全性：https://
...
1. `glog`
    - 增加支持`Context`上下文变量的日志打印特性：https://
...
1. `guid`
    - 新增`guid`模块，用于高效轻量级的唯一字符串生成：https://
...
1. `gvalid`
    - 所有默认的错误提示改为了英文；
    - 错误提示的配置改为了通过`i18n`来配置实现，以便支持国际化；
