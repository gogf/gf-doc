...
1. `ghttp`
    - `Client`
        - 客户端`Client`增加`GetVar/PutVar/PostVar`等`*Var`请求方法，用于发起`HTTP`请求获取内容之后直接返回泛型对象，方便类型转换，特别是针对于返回`XML/JSON`的结果处理将会更加简便：https://goframe.org/net/ghttp/client/demo/index
        - 客户端`Client`增加`SetProxy/Proxy`方法，用于设置客户端代理，支持`HTTP/Socket5`代理类型：https://goframe.org/net/ghttp/client/demo/proxy
        - 客户端`Client`增加`SetRedirectLimit/RedirectLimit`方法，用于设置页面跳转数量限制。
    - `Request`
        - `Request`请求对象增加`GetHeader`方法，用于获取指定`Header`参数。
        - `Request`请求对象增加`GetRemoteIp`方法，用于获取请求客户端IP。在IP白名单限制时应当使用`GetRemoteIp`而不是`GetClientIp`进行判断，后者可以通过`Header`伪造。
